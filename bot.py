# bot.py
# Full Club Auction Bot (Final Ultimate Build - Part 1 of 3)
...
# Log Channels (Ensure these IDs are correct)
LOG_CHANNELS = {
...
# Currency Specific Emojis (Update IDs as needed)
E_SHINY = "<a:shiny:1443251827490684938>" 
E_PC = "<:pc:1443250889266565190>"      
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if val == "Economy":
            ...
            *Example:* `.ww 50k` (Removes $50,000 forever)
            ...
            `•` **/daily**
            Claim your daily reward (Cash $). You must have sent **100 messages** in the chat channel today.
            *Example:* `/daily`
            ...
            **{E_STARS} Shop & Inventory System**
            ...
            `•` **/shop**
            Open the interactive shop menu. Select a category (Items, Shiny Coins, Pokemon, Mystery Boxes, User Market) to view stock.
            *Usage:* `/shop`
            ...
            *Example:* `/pinfo 105` (Views Pokemon ID 105 from shop)
            ...
            **4. Shop & User Market**
            • **3 Currencies:** Cash ($), Shiny Coins (SC), Pokécoins (PC).
...
async def daily(ctx):
    ...
    stats = daily_stats_col.find_one({"user_id": ctx.author.id, "date": today})
...
async def groupbid(ctx, group_name: str, amount: HumanAmount, item_type: str, item_id: int, club_name: str = None):
    ...
    bids_col.insert_one({"bidder": f"group:{gname}", "amount": amount, "item_type": item_type, "item_id": item_id})
...
async def clubinfo(ctx, *, club_name_or_id: str):
    ...
    embed.add_field(name=f"{E_STAR} Owner", value=owner, inline=True)
...
async def registerduelist(ctx, username: str, base_price: int, salary: int):
    ...
    duelists_col.insert_one({"id": did, "discord_user_id": str(ctx.author.id), "username": username, "base_price": base_price, "expected_salary": salary, "avatar_url": ctx.author.avatar.url if ctx.author.avatar else "", "owned_by": None, "club_id": None})
...
@bot.hybrid_command(name="retireduelist", description="Retire your duelist profile.")
...
@bot.hybrid_command(name="deductsalary", description="Owner: Match Penalty (15%).")
...
@bot.hybrid_command(name="buycoins", description="Convert Cash to Shiny Coins ($100 = 1 SC).")
...
async def buy(ctx, item_id: str, quantity: int = 1, coupon: str = None):
    ...
    if item_id.startswith("U"):
        ...
        if chn:
            ...
            b1 = Button(label="Approve", style=discord.ButtonStyle.green); b1.callback = approve_cb
            b2 = Button(label="No Tax (Premium)", style=discord.ButtonStyle.blurple); b2.callback = premium_approve_cb
            b3 = Button(label="Decline", style=discord.ButtonStyle.red); b3.callback = decline_cb
...
@bot.hybrid_command(name="sellpokemon", description="List a Pokemon on User Market (PC).")
...
@bot.hybrid_command(name="addshopitem", description="Admin: Add item.")
...
@bot.hybrid_command(name="addpokemon", description="Admin: Add Pokemon (Scrape).")
@commands.has_permissions(administrator=True)
...
@bot.hybrid_command(name="addshinycoins", description="Admin: Add Shiny Coins.")
@commands.has_permissions(administrator=True)
