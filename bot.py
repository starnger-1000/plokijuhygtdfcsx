# bot.py
# Full Club Auction Bot (Final Release - Blueprint 5.2 - Part 1 of 5)
# PART 1: Infrastructure, Database, Assets, Helpers, and ALL UI Systems
...
# ==============================================================================
#  2. ASSETS: PREMIUM EMOJIS & BANTER (STRICT BLUEPRINT 5.2)
# ==============================================================================
...
# Currency Specific Emojis (Updated to Blueprint 5.2)
...
# ==============================================================================
#  7. MARKET & AUCTION COMMANDS
# ==============================================================================

@bot.hybrid_command(name="placebid", aliases=["pb"], description="Place a bid on a Club or Duelist.")
async def placebid(ctx, amount: HumanAmount, item_type: str, item_id: int, club_name: str = None):
    if bidding_frozen: return await ctx.send(embed=create_embed("Frozen", f"{E_DANGER} Auctions are currently frozen.", 0xff0000))
    ...
    if item_type == "duelist":
        ...
        if not club_name: return await ctx.send(embed=create_embed("Error", "You must provide your club name to sign a player.", 0xff0000))
        ...
        if not allowed: return await ctx.send(embed=create_embed("Error", "You (or your group) do not own this club.", 0xff0000))
    ...
    if item_type == "club":
        ...
        is_active = (item_type, str(item_id)) in active_timers
        if c.get("owner_id") and not is_active: return await ctx.send(embed=create_embed(f"{E_ALERT} Sold Out", f"{E_ERROR} This club is already owned.", 0xff0000))
    ...
    # --- Financial Check ---
    w = get_wallet(ctx.author.id)
    if not w or w.get("balance", 0) < amount: return await ctx.send(embed=create_embed("Error", "Insufficient funds in Cash ($) wallet.", 0xff0000))
    
    # --- Minimum Bid Check ---
    current_bid = get_current_bid(item_type, item_id)
    req = min_required_bid(current_bid)
    if amount < req: return await ctx.send(embed=create_embed("Bid Error", f"Minimum bid required is **${req:,}**", 0xff0000))
    
    # --- Execute Bid ---
    bids_col.insert_one({
        "bidder": str(ctx.author.id), 
        "amount": amount, 
        "item_type": item_type, 
        "item_id": int(item_id), 
    ...
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Bid Placed", f"Bid of **${amount:,}** accepted for {item_type} #{item_id}.", 0x2ecc71))
...
async def groupbid(ctx, group_name: str, amount: HumanAmount, item_type: str, item_id: int, club_name: str = None):
    if bidding_frozen: return await ctx.send(embed=create_embed("Frozen", "Auctions are currently frozen.", 0xff0000))
    ...
    # Membership Check
    if not group_members_col.find_one({"group_name": gname, "user_id": str(ctx.author.id)}): return await ctx.send(embed=create_embed("Error", "You are not a member of this group.", 0xff0000))
    ...
    if item_type == "club":
         ...
         if clubs_col.find_one({"owner_id": f"group:{gname}"}): return await ctx.send(embed=create_embed("Error", "Your group already owns a club.", 0xff0000))

    # Funds Check
    if g["funds"] < amount: return await ctx.send(embed=create_embed("Error", f"Insufficient group funds. Group has ${g['funds']:,}", 0xff0000))
    
    # Execute
    bids_col.insert_one({"bidder": f"group:{gname}", "amount": amount, "item_type": item_type, "item_id": int(item_id)})
    log_user_activity(ctx.author.id, "Bid", f"Group {gname} bid ${amount:,} on {item_type} {item_id}")
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Group Bid", f"Group **{group_name}** successfully bid **${amount:,}**.", 0x2ecc71))
    
    schedule_auction_timer(item_type, item_id, ctx.channel.id)

@bot.hybrid_command(name="sellclub", aliases=["sc"], description="Sell your club to the market or a user.")
async def sellclub(ctx, club_name: str, buyer: discord.Member = None):
    c = clubs_col.find_one({"name": {"$regex": f"^{club_name}$", "$options": "i"}})
    if not c: return await ctx.send(embed=create_embed("Error", "Club not found.", 0xff0000))
    
    if str(ctx.author.id) != c.get("owner_id"): return await ctx.send(embed=create_embed("Error", "You don't own this club.", 0xff0000))
    
    val = c["value"]
    target = buyer.mention if buyer else "The Market"
    
    await ctx.send(embed=create_embed(f"{E_ALERT} Confirm Sale", f"Are you sure you want to sell **{c['name']}** to {target} for **${val:,}**?\nType `yes` or `no`.", 0xe67e22))
    ...
    if msg.content.lower() == 'no': return await ctx.send(embed=create_embed("Info", "Sale cancelled.", 0x95a5a6))
    ...
    if buyer:
        ...
        if not bw or bw.get("balance", 0) < val: return await ctx.send(embed=create_embed("Error", "Buyer has insufficient funds.", 0xff0000))
    ...
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Sold", f"Club sold successfully for **${val:,}**.", 0x2ecc71))

@bot.hybrid_command(name="sellshares", aliases=["ss"], description="Sell your shares in a group-owned club.")
async def sellshares(ctx, club_name: str, buyer: discord.Member, percentage: int):
    c = clubs_col.find_one({"name": {"$regex": f"^{club_name}$", "$options": "i"}})
    if not c: return await ctx.send(embed=create_embed("Error", "Club not found.", 0xff0000))
    
    owner_str = c.get("owner_id", "")
    if not owner_str.startswith("group:"): return await ctx.send(embed=create_embed("Error", "This club is not owned by a group.", 0xff0000))
    
    gname = owner_str.replace("group:", "")
    seller = group_members_col.find_one({"group_name": gname, "user_id": str(ctx.author.id)})
    
    if not seller or seller["share_percentage"] < percentage: return await ctx.send(embed=create_embed("Error", "You do not own enough shares.", 0xff0000))
    
    val = int(c["value"] * (percentage / 100))
    
    await ctx.send(embed=create_embed(f"{E_ALERT} Confirm Share Sale", f"{buyer.mention}, do you want to buy **{percentage}%** shares of **{c['name']}** for **${val:,}**?\nType `yes` or `no`.", 0xe67e22))
    ...
    if msg.content.lower() == 'yes':
        ...
        await ctx.send(embed=create_embed(f"{E_SUCCESS} Sold", "Shares transferred successfully.", 0x2ecc71))
    else:
        await ctx.send(embed=create_embed("Info", "Transaction cancelled.", 0x95a5a6))

@bot.hybrid_command(name="marketbuy", aliases=["mb"], description="Instantly buy an unowned club.")
async def marketbuy(ctx, club_id: int, group_name: str = None):
    ...
    if group_name:
        g = groups_col.find_one({"name": group_name.lower()})
        if not g: return await ctx.send(embed=create_embed("Error", "Group not found.", 0xff0000))
        if not group_members_col.find_one({"group_name": group_name.lower(), "user_id": str(ctx.author.id)}): return await ctx.send(embed=create_embed("Error", "Not a member.", 0xff0000))
        ...
        if g["funds"] < price: return await ctx.send(embed=create_embed("Error", "Insufficient group funds.", 0xff0000))
        
        buyer_id = f"group:{group_name.lower()}"
        is_group = True
    else:
        prof = profiles_col.find_one({"user_id": str(ctx.author.id)})
        if prof and prof.get("owned_club_id"): return await ctx.send(embed=create_embed("Error", "You already own a club.", 0xff0000))
        
        w = get_wallet(ctx.author.id)
        if not w or w.get("balance", 0) < price: return await ctx.send(embed=create_embed("Error", "Insufficient personal funds.", 0xff0000))
        
    view = MarketConfirmView(ctx, club_id, price, buyer_id, is_group)
    await ctx.send(embed=create_embed(f"{E_AUCTION} Confirm Purchase", f"Do you want to buy **{c['name']}** for **${price:,}**?", 0xe67e22), view=view)

@bot.hybrid_command(name="marketlist", aliases=["ml"], description="List all unowned clubs for sale.")
async def marketlist(ctx):
    clubs = list(clubs_col.find({"owner_id": None}).sort("value", -1))
    
    if not clubs: return await ctx.send(embed=create_embed("Market Empty", "No clubs available for instant purchase.", 0x95a5a6))
...
@bot.hybrid_command(name="clubinfo", aliases=["ci"], description="Get detailed stats of a club.")
async def clubinfo(ctx, *, club_name_or_id: str):
    ...
    owner_display = "Unowned"
    shareholder_text = ""
    raw_owner = c.get('owner_id')
    
    if raw_owner:
        if raw_owner.startswith('group:'): 
            gname = raw_owner.replace('group:', '').title()
            owner_display = f"Group: {gname}"
            # Fetch Shareholders
            raw_gname_db = raw_owner.replace('group:', '').lower()
            members = list(group_members_col.find({"group_name": raw_gname_db}))
            shares = []
            for m in members:
                 try: u = await bot.fetch_user(int(m['user_id'])); name = u.name
                 except: name = "Unknown"
            ...
            try:
                owner_user = await bot.fetch_user(int(raw_owner))
                owner_display = f"User: {owner_user.display_name}"
            except: owner_display = "Unknown User"
...
@bot.hybrid_command(name="leaderboard", aliases=["lb"], description="View club leaderboard.")
async def leaderboard(ctx):
    clubs = list(clubs_col.find().sort([("total_wins", -1)]))
    data = []
    for i, c in enumerate(clubs):
        data.append((f"{E_CROWN} #{i+1} {c['name']}", f"{E_FIRE} {c.get('total_wins',0)} Wins | {E_MONEY} ${c['value']:,}"))
    view = Paginator(ctx, data, f"{E_CROWN} Club Leaderboard", 0xf1c40f)
    await ctx.send(embed=view.get_embed(), view=view)

@bot.hybrid_command(name="clublevel", description="Check club level.")
...
@bot.hybrid_command(name="marketpanel", description="View market visualization.")
...
async def retireduelist(ctx):
    ...
    if d.get("club_id"):
        # Could add check here to prevent signed players retiring without permission
        pass 
...
async def setclubmanager(ctx, name: str, mem: discord.Member):
    ...
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Manager Set", f"{mem.mention} is now manager.", 0x2ecc71))
