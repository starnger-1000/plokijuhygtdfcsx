# bot.py
# Full Club Auction Bot (MongoDB + Premium Embeds + All Features)
...
async def withdrawwallet(ctx, amount: int):
    ...
    w = wallets_col.find_one({"user_id": str(ctx.author.id)})
    if not w or w["balance"] < amount: return await ctx.send(embed=create_embed("Error", "Insufficient funds.", 0xff0000))
...
async def sellclub(ctx, club_name: str, buyer: discord.Member = None):
    ...
    if buyer:
        ...
        if not bw or bw["balance"] < val: return await ctx.send(embed=create_embed("Error", "Buyer broke.", 0xff0000))
...
async def battleresult(ctx, battle_id: int, winner_name: str):
    ...
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Result", f"Winner: **{wc['name']}** (+${WIN_VALUE_BONUS:,})", 0x2ecc71))
...
async def placebid(ctx, amount: int, item_type: str, item_id: int, club_name: str = None):
    ...
    if item_type == "duelist":
        ...
        c = clubs_col.find_one({"name": {"$regex": f"^{club_name}$", "$options": "i"}})
        if not c or str(ctx.author.id) != c.get("owner_id"): return await ctx.send(embed=create_embed("Error", "You don't own this club.", 0xff0000))
