# bot.py
# Full Club Auction Bot (Final Production Build)
...
# Banter Lines (Dynamic Placeholders: {winner}, {loser}, {w_emoji}, {l_emoji})
BATTLE_BANTER = [
    "Absolute demolition! **{winner}** tore **{loser}** apart like homework due tomorrow. {w_emoji} What a massacre! {l_emoji}",
    "**{winner}** owned the pitch today — sent **{loser}** home with a souvenir bag full of tears. {l_emoji}",
    "That wasn’t a match… that was a public execution. RIP **{loser}**, thoughts and prayers. {w_emoji}",
    "**{loser}** came to play football, **{winner}** came to play career-ending football. {w_emoji}",
    "That performance was so one-sided, even the referee felt bad for **{loser}**. {l_emoji}",
    "**{winner}** delivered a masterclass — **{loser}** just attended the lecture. {w_emoji}",
    "Someone call 911, we just witnessed a crime against **{loser}**. {l_emoji}",
    "Fortress defended! **{winner}** protected their home like it was the national treasury. {w_emoji}",
    "Home crowd roaring, scoreboard soaring — **{winner}** showed **{loser}** the door. {l_emoji}",
    "Welcome to our home — shoes off, ego off, points taken. Try again next time **{loser}**. {w_emoji}",
    "Silence in the stadium — **{winner}** just robbed **{loser}** in their own house. {w_emoji}",
    "**{winner}** turned **{loser}**'s home ground into a training session. {l_emoji}",
    "Fans left early, stadium empty — **{winner}** evacuated **{loser}**'s hopes. {w_emoji}",
    "From hopeless to ruthless — **{winner}** just turned the match into a movie! {w_emoji}",
    "Heartbreak for **{loser}**! Just when they thought they had it, **{winner}** said plot twist. {l_emoji}",
    "Comeback kings! **{winner}** woke up and chose violence. {w_emoji}",
    "**{loser}** tried their best, but sometimes best is not enough. {l_emoji}",
    "Fought hard, but went home harder — rough night for **{loser}**. {l_emoji}",
    "Painful defeat — that last moment hit **{loser}** harder than a tax bill. {l_emoji}",
    "**{loser}** bottled it like a soda factory. {w_emoji}",
    "They had the lead… and then **{loser}** donated it to **{winner}**. {l_emoji}",
    "That collapse by **{loser}** was sponsored by gravity. {w_emoji}",
    "A draw that felt like a loss for **{loser}** and a rescue mission for **{winner}**. {l_emoji}",
    "Balanced scoreline, unbalanced emotions. {w_emoji} {l_emoji}"
]

# Emojis
WINNER_REACTIONS = [
    1443996271990935552, 1443996261941383178, 1443996171071914177, 1443996142382612662, 
    1443996193012187217, 1443996181733834803, 1443996134031753306, 1443996236356259861, 
    1443996221608824922, ":7833dakorcalmao:", ":33730ohoholaugh:", ":44158laughs:", 
    ":69692pepewine:", ":954110babythink:", ":10436batmanlaugh:"
]

LOSER_REACTIONS = [
    1443996214805790871, 1443996269113643028, 1443996159080403174, 1443996188951973999, 
    1443996139362844783, 1443996248871928090, "<:192978sadchinareact:1443996152772038678>", 
    "<:26955despair:1443996205028999360>", "<:8985worldcup:1443996229620203712>"
]
...
def resolve_emoji(item):
    """Helper to convert ID integers or :name: strings to Discord Emoji Objects"""
    # 1. If Integer ID or String ID -> Fetch Emoji Object
    if isinstance(item, int) or (isinstance(item, str) and item.isdigit()):
        e = bot.get_emoji(int(item))
        if e: return str(e)
        return f"<:e:{item}>" # Best-effort fallback
    
    # 2. If Alias ":name:" -> Search Bot Emojis
    if isinstance(item, str) and item.startswith(":") and item.endswith(":"):
        name = item.strip(":")
        e = discord.utils.get(bot.emojis, name=name)
        if e: return str(e)
    
    # 3. Return as-is (Assume valid formatted string)
    return str(item)
...
async def finalize_auction(item_type: str, item_id: int, channel_id: int):
    ...
    if winner_bid:
        ...
        if item_type == "club":
            ...
            if channel: await channel.send(embed=embed_win)
            await send_log("club", embed_win)
            ...
            if channel: await channel.send(embed=embed_d)
            await send_log("duelist", embed_d)
    else:
...
async def payout(ctx, user: discord.Member, amount: int, *, reason: str):
    ...
    if not w or w.get("balance", 0) < amount: return await ctx.send(embed=create_embed("Error", f"{E_ERROR} User has insufficient funds to cash out.", 0xff0000))
...
async def withdraw(ctx, group_name: str, amount: int):
    ...
    embed_log = create_embed(f"{E_MONEY} Withdraw Log", f"**User:** {ctx.author.mention}\n**From:** {group_name}\n**Amount:** ${amount:,}\n**Date:** {datetime.now().strftime('%Y-%m-%d')}", 0xe74c3c)
    await send_log("withdraw", embed_log)
...
async def battleresult(ctx, battle_id: int, winner_name: str):
    ...
    winner_emoji = resolve_emoji(random.choice(WINNER_REACTIONS))
    loser_emoji = resolve_emoji(random.choice(LOSER_REACTIONS))
    embed_log = create_embed(f"{E_FIRE} Match Result", f"{E_WINNER_TROPHY} **Winner:** {wc['name']} {winner_emoji}\n{E_LOSER_MARK} **Loser:** {lc['name']} {loser_emoji}\n\n_{banter.replace('Team A', wc['name']).replace('Team B', lc['name'])}_", 0xe74c3c)
    await send_log("battle", embed_log)
    await ctx.send(embed=embed_log)
