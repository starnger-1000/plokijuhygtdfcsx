...
# ==============================================================================
#  TAG STATUS & REWARD SYSTEM (Name & Bio Check)
# ==============================================================================

async def check_user_tag(ctx, member):
    """
    Checks if the user has the Tag in:
    1. Server Nickname
    2. Global Display Name
    3. User Bio (About Me)
    """
    tag = SERVER_TAG_STRING.lower()
    
    # 1. Check Server Nickname
    if member.nick and tag in member.nick.lower(): return True
    
    # 2. Check Global Name / Username
    if member.global_name and tag in member.global_name.lower(): return True
    if tag in member.name.lower(): return True
    
    # 3. Check Bio (Requires API Fetch)
    try:
        full_user = await ctx.bot.fetch_user(member.id)
        if full_user.bio and tag in full_user.bio.lower(): return True
    except: pass
            
    return False

@bot.hybrid_command(name="tagstatus", aliases=["ts"], description="Check your Tag Streak.")
async def tagstatus(ctx):
    uid = str(ctx.author.id)
    w = get_wallet(uid)
    
    # 1. Check Tag
    has_tag = await check_user_tag(ctx, ctx.author)
    
    # 2. Check Admin Pause
    is_paused = w.get("tag_paused", False)
    
    # 3. Calculate Streak
    now = datetime.now()
    started_at = w.get("tag_started_at")
    
    status_msg = ""
    streak_msg = ""
    
    if is_paused:
        status_msg = f"{E_DANGER} **Paused by Admin**"
        streak_msg = "Frozen"
    elif not has_tag:
        if started_at: 
            # Reset streak if tag is lost
            wallets_col.update_one({"user_id": uid}, {"$set": {"tag_started_at": None}})
            started_at = None
        status_msg = f"{E_ERROR} **Inactive**\nAdd `{SERVER_TAG_STRING}` to your Name or Bio to activate!"
        streak_msg = "0 Days"
    else:
        # Tag is Active
        if not started_at:
            # New Streak
            wallets_col.update_one({"user_id": uid}, {"$set": {"tag_started_at": now}})
            started_at = now
        ...
        status_msg = f"{E_ACTIVE} **Active**"
        streak_msg = f"**{days} Days, {hours} Hours**"

    # 4. Check Daily Message Requirement
    today = datetime.now().strftime("%Y-%m-%d")
    stats = daily_stats_col.find_one({"user_id": uid, "date": today})
    msg_count = stats["count"] if stats else 0
    msg_req_display = f"{E_GOLD_TICK} Completed" if msg_count >= 100 else f"{E_ALERT} {msg_count}/100 Messages"

    # 5. Build Embed
    embed = discord.Embed(title=f"{E_CROWN} Tag Status", color=0x3498db)
    ...
    embed.add_field(name="Current Streak", value=streak_msg, inline=True)
    embed.add_field(name="Daily Chat", value=msg_req_display, inline=True)
    
    # 6. Next Claim Info
    last_claim = w.get("last_tag_claim")
    if last_claim:
        next_claim_ts = int((last_claim + timedelta(hours=24)).timestamp())
        embed.add_field(name=f"{E_TIMER} Next Reward", value=f"<t:{next_claim_ts}:R>", inline=False)
    else:
        embed.add_field(name=f"{E_TIMER} Next Reward", value="**Ready Now!**" if has_tag else "Start streak first", inline=False)

    await ctx.send(embed=embed)

@bot.hybrid_command(name="tagclaim", aliases=["tc"], description="Claim your Daily Tag Reward.")
async def tagclaim(ctx):
    uid = str(ctx.author.id)
    w = get_wallet(uid)
    now = datetime.now()

    # 1. Verify Tag
    if not await check_user_tag(ctx, ctx.author):
        return await ctx.send(embed=create_embed("Error", f"{E_ERROR} You don't have `{SERVER_TAG_STRING}` in your Name or Bio.", 0xff0000))
    
    # 2. Verify Pause
    if w.get("tag_paused", False):
        return await ctx.send(embed=create_embed("Error", f"{E_DANGER} Your tag status is paused.", 0xff0000))

    # 3. Verify Messages
    today = now.strftime("%Y-%m-%d")
    stats = daily_stats_col.find_one({"user_id": uid, "date": today})
    if not stats or stats["count"] < 100:
        return await ctx.send(embed=create_embed("Requirement Met Not", f"{E_ALERT} You need **100 messages** in <#{CHAT_CHANNEL_ID}> today.", 0xff0000))

    # 4. Check Cooldown (24h)
    last_claim = w.get("last_tag_claim")
    if last_claim and (now - last_claim) < timedelta(hours=24):
        next_claim_ts = int((last_claim + timedelta(hours=24)).timestamp())
        return await ctx.send(embed=create_embed("Cooldown", f"{E_TIMER} Claim available <t:{next_claim_ts}:R>.", 0xff0000))

    # 5. Reward: Give "Tag Mystery Box"
    # Ensure Tag Streak is active
    if not w.get("tag_started_at"):
        wallets_col.update_one({"user_id": uid}, {"$set": {"tag_started_at": now}})

    inventory_col.update_one(
        {"user_id": uid, "name": TAG_BOX_NAME},
        {"$inc": {"quantity": 1}, "$set": {"name": TAG_BOX_NAME, "type": "Mystery Box"}},
        upsert=True
    )
    
    wallets_col.update_one({"user_id": uid}, {"$set": {"last_tag_claim": now}})
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Reward Claimed", f"You received **1x {TAG_BOX_NAME}**! {E_GIVEAWAY}\nUse `/open` to check it.", 0x2ecc71))

@bot.hybrid_command(name="open", description="Open a Tag Mystery Box.")
async def open_box(ctx):
    uid = str(ctx.author.id)
    
    # 1. Check Inventory for TAG_BOX
    box = inventory_col.find_one({"user_id": uid, "name": TAG_BOX_NAME})
    
    if not box or box.get("quantity", 0) < 1:
        return await ctx.send(embed=create_embed("Error", f"{E_ERROR} You don't have a **{TAG_BOX_NAME}**.", 0xff0000))
    
    # 2. RNG Reward ($30k - $150k)
    reward = random.randint(30000, 150000)
    
    # 3. Transaction
    inventory_col.update_one({"_id": box["_id"]}, {"$inc": {"quantity": -1}}) # Remove 1 Box
    wallets_col.update_one({"user_id": uid}, {"$inc": {"balance": reward}}) # Add Cash
    
    # 4. Cleanup Empty Box
    updated_box = inventory_col.find_one({"_id": box["_id"]})
    if updated_box and updated_box["quantity"] <= 0: 
        inventory_col.delete_one({"_id": box["_id"]})

    # 5. Premium Reveal
    embed = discord.Embed(title=f"{E_GIVEAWAY} Box Opened!", description=f"You opened a **{TAG_BOX_NAME}** and found...", color=0xFF69B4)
    embed.add_field(name="Reward", value=f"{E_MONEY} **${reward:,}**", inline=False)
    
    await ctx.send(embed=embed)

@bot.hybrid_command(name="pausetag", description="Admin: Pause/Unpause a user's tag status.")
@commands.has_permissions(administrator=True)
async def pausetag(ctx, member: discord.Member):
    uid = str(member.id)
    w = get_wallet(uid)
    
    current_status = w.get("tag_paused", False)
    new_status = not current_status
    
    wallets_col.update_one({"user_id": uid}, {"$set": {"tag_paused": new_status}})
    
    state = "Paused" if new_status else "Resumed"
    emoji = E_DANGER if new_status else E_SUCCESS
    
    await ctx.send(embed=create_embed(f"{emoji} Tag Status Updated", f"{member.mention}'s tag status has been **{state}**.", 0x3498db))
