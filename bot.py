# bot.py
# Full Club Auction & Pokémon Shop Bot (Certified Final Production Build v5.0)
# Part 1 of 2 - Core, Economy, Club Market & Football
...
LOG_CHANNELS = {
    ...
    "shop_log": 1446017729340379246, # Pokemon Sold-out Log
...
# ===========================
#   GROUP 3.5/3.6: INTERACTIVE SHOP
# ===========================

class ShopCategoryView(discord.ui.View):
    def __init__(self, ctx, shop_type):
        super().__init__(timeout=60)
        self.ctx = ctx
    ...
    async def show_category(self, interaction, category):
        ...
        if not items: return await interaction.response.send_message(f"No items in **{category.title()}**.", ephemeral=True)
        
        data = []
        for i in items: data.append((f"{i['name']}", f"**Price:** {i['price']:,} {emoji_c}\n**ID:** {i['id']}"))
        view = Paginator(self.ctx, data, f"{self.shop_type.title()} Shop - {category.title()}", 0x3498db)
        await interaction.response.send_message(embed=view.get_embed(), view=view, ephemeral=True)

    @discord.ui.button(label="Shiny", style=discord.ButtonStyle.primary, emoji="✨")
    async def shiny(self, interaction: discord.Interaction, button: discord.ui.Button): await self.show_category(interaction, "shiny")
    @discord.ui.button(label="Rare", style=discord.ButtonStyle.primary, emoji="🎁")
    async def rare(self, interaction: discord.Interaction, button: discord.ui.Button): await self.show_category(interaction, "rare")
    @discord.ui.button(label="Regional", style=discord.ButtonStyle.primary, emoji="🌏")
    async def regional(self, interaction: discord.Interaction, button: discord.ui.Button): await self.show_category(interaction, "regional")
    @discord.ui.button(label="Common", style=discord.ButtonStyle.primary, emoji="📦")
    async def common(self, interaction: discord.Interaction, button: discord.ui.Button): await self.show_category(interaction, "common")
...
class ShopTypeView(discord.ui.View):
    ...
    async def admin_shop(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.ctx.author.id: return
        view = ShopCategoryView(self.ctx, "admin")
        await interaction.response.send_message(embed=create_embed(f"{E_SHINY} Admin Shop", "Select Category:", 0xe74c3c), view=view, ephemeral=True)
    ...
    async def user_shop(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.ctx.author.id: return
        view = ShopCategoryView(self.ctx, "user")
        await interaction.response.send_message(embed=create_embed(f"{E_PC} User Shop", "Select Category:", 0x2ecc71), view=view, ephemeral=True)

@bot.hybrid_command(name="shop", description="Open Shop Menu.")
async def shop(ctx):
    view = ShopTypeView(ctx)
    await ctx.send(embed=create_embed(f"{E_ITEMBOX} Pokémon Market", "Select a shop type:", 0x3498db), view=view)

@bot.hybrid_command(name="shop_import", aliases=["si"], description="Admin: Add item (Cat: shiny/rare/reg/common).")
@commands.has_permissions(administrator=True)
async def shop_import(ctx, name: str, price: int, category: str, is_shiny: bool = False, image: discord.Attachment = None):
    ...
    image_url = image.url if image else None
    shop_items_col.insert_one({"id": item_id, "name": name, "price": price, "currency": "shiny", "seller_id": "ADMIN", "is_shiny": is_shiny, "image_url": image_url, "sold": False, "timestamp": datetime.now(), "category": cat})
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Added", f"**{name}** added to Admin Shop.\nPrice: {price:,} {E_SHINY}", 0x2ecc71, image=image_url))

@bot.hybrid_command(name="user_shop_add", aliases=["usa"], description="List item (Cat: shiny/rare/reg/common).")
async def user_shop_add(ctx, price: int, category: str):
    ...
    try:
        ...
        shop_items_col.insert_one({
            ...
            "seller_id": str(ctx.author.id), "is_shiny": False, "image_url": image_url, "sold": False, "timestamp": datetime.now(), "category": cat
        })
        await ctx.send(embed=create_embed(f"{E_SUCCESS} Listed", f"**{name}** listed in {cat} for {price:,} {E_PC}!", 0x2ecc71, image=image_url))
    except asyncio.TimeoutError: await ctx.send("Timeout.")

@bot.hybrid_command(name="shop_buy", aliases=["sb"], description="Buy item.")
async def shop_buy(ctx, item_id: int):
    item = shop_items_col.find_one({"id": item_id, "sold": False})
    if not item: return await ctx.send("Item not found/sold.")
    w = wallets_col.find_one({"user_id": str(ctx.author.id)})
    curr = "shiny_coins" if item['currency'] == "shiny" else "pc"
    bal = w.get(curr, 0) if w else 0
    if bal < item['price']: return await ctx.send("Insufficient funds.")
    wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {curr: -item['price']}})
    ...
    shop_items_col.update_one({"id": item_id}, {"$set": {"sold": True, "buyer_id": str(ctx.author.id)}})
    ...
    if log_ch: await log_ch.send(embed=create_embed(f"{E_SUCCESS} Sold", f"Item {item['name']} sold to {ctx.author.mention}", 0x2ecc71))
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Bought", f"You bought **{item['name']}**!", 0x2ecc71))

@bot.hybrid_command(name="shop_remove", aliases=["sr"], description="Admin: Remove item.")
@commands.has_permissions(administrator=True)
async def shop_remove(ctx, item_id: int):
    res = shop_items_col.delete_one({"id": item_id})
    if res.deleted_count > 0: await ctx.send("Removed.")
...
async def user_shop_remove(ctx, item_id: int):
    res = shop_items_col.delete_one({"id": item_id, "seller_id": str(ctx.author.id)})
    if res.deleted_count > 0: await ctx.send("Removed.")
    else: await ctx.send("Not found/Not yours.")

@bot.hybrid_command(name="shop_setprice", aliases=["ssp"], description="Admin: Set Price.")
@commands.has_permissions(administrator=True)
async def shop_setprice(ctx, item_id: int, price: int):
    shop_items_col.update_one({"id": item_id}, {"$set": {"price": price}})
...
async def inventory(ctx):
    ...
    if not items: return await ctx.send("Empty.")
    data = [(i['item_name'], f"Price: {i['price']:,}") for i in items]
    view = Paginator(ctx, data, "Inventory", 0x3498db)
    await ctx.send(embed=view.get_embed(), view=view)

@bot.hybrid_command(name="daily", aliases=["claim"], description="Daily (100 msgs req).")
async def daily(ctx):
    today = datetime.now().strftime("%Y-%m-%d")
    data = message_counts_col.find_one({"user_id": str(ctx.author.id), "date": today})
    count = data.get("count", 0) if data else 0
    if count < DAILY_MSG_REQ: return await ctx.send(f"Need {DAILY_MSG_REQ} msgs (Current: {count})")
    user = wallets_col.find_one({"user_id": str(ctx.author.id)})
    last = user.get("last_daily")
    if last and (datetime.now() - last) < timedelta(hours=24): return await ctx.send("Cooldown.")
    wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {"balance": 10000, "shiny_coins": 5}, "$set": {"last_daily": datetime.now()}}, upsert=True)
    await ctx.send(embed=create_embed(f"{E_GIVEAWAY} Claimed", f"+$10,000 {E_MONEY}\n+5 {E_SHINY}", 0x2ecc71))

@bot.hybrid_command(name="pc_deposit", aliases=["pcd"], description="Deposit PC.")
async def pc_deposit(ctx):
    await ctx.send("Run `<@716390085896962058> bal`.")
    def check(m): return m.channel == ctx.channel and m.author.id == 716390085896962058 and m.content
    try:
        msg = await bot.wait_for('message', check=check, timeout=30)
        match = re.search(r'(\d+)\s*pc', msg.content.lower().replace(",",""))
        if not match: return await ctx.send("Error reading.")
        bal = int(match.group(1))
        await ctx.send(f"Found {bal} PC. Enter amount.")
        amt = await bot.wait_for('message', check=lambda m: m.author==ctx.author, timeout=30)
        amount = int(amt.content.replace(",",""))
        if amount > bal: return await ctx.send("Not enough.")
        wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {"pc": amount}}, upsert=True)
        await ctx.send(f"Deposited {amount} PC.")
    except: await ctx.send("Timeout/Error.")

@bot.hybrid_command(name="mystery_buy", aliases=["mb"], description="Buy Box.")
async def mystery_buy(ctx, type: str):
    key = type.lower()
    if key not in MYSTERY_CONFIG: return await ctx.send("Invalid type.")
    price = MYSTERY_CONFIG[key]['price']
    w = wallets_col.find_one({"user_id": str(ctx.author.id)})
    if not w or w.get("shiny_coins", 0) < price: return await ctx.send("Insufficient Shiny.")
    wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {"shiny_coins": -price}})
    await ctx.send(f"Bought {key} box!")

@bot.hybrid_command(name="create_coupon", aliases=["cc"], description="Admin: Coupon.")
@commands.has_permissions(administrator=True)
async def create_coupon(ctx, code: str, disc: int, uses: int):
    coupons_col.insert_one({"code": code, "discount": disc, "uses": uses})
    await ctx.send("Created.")

@bot.hybrid_command(name="coupon", description="Use Coupon.")
async def coupon(ctx, code: str):
    c = coupons_col.find_one({"code": code})
    if not c or c['uses'] <= 0: return await ctx.send("Invalid.")
    coupons_col.update_one({"_id": c['_id']}, {"$inc": {"uses": -1}})
    await ctx.send(f"Applied {c['discount']}% off.")

@bot.hybrid_command(name="create_redeem", aliases=["crc"], description="Admin: Redeem.")
@commands.has_permissions(administrator=True)
async def create_redeem(ctx, type: str, amt: int):
    code = ''.join(random.choices(string.ascii_uppercase, k=8))
    redeem_codes_col.insert_one({"code": code, "type": type, "amount": amt, "claimed": False})
    await ctx.send(f"Code: {code}")

@bot.hybrid_command(name="redeem", aliases=["rcode"], description="Claim Code.")
async def redeem(ctx, code: str):
    r = redeem_codes_col.find_one({"code": code, "claimed": False})
    if not r: return await ctx.send("Invalid.")
    redeem_codes_col.update_one({"_id": r['_id']}, {"$set": {"claimed": True}})
    wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {"pc" if r['type']=='pc' else "shiny_coins": r['amount']}}, upsert=True)
...
class HelpView(discord.ui.View):
    ...
    @discord.ui.button(label="Updates (v4.0)", style=discord.ButtonStyle.primary, emoji=discord.PartialEmoji.from_str(E_BOOST))
    async def updates_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        desc = (f"{E_STARS} **Latest Updates**\n{E_GOLD_TICK} **Interactive Shop:** New .shop menu with categories.\n{E_GOLD_TICK} **User Listings:** Added Category support to .usa.\n{E_GOLD_TICK} **Daily Task:** Chat in #general to claim $10k.\n{E_GOLD_TICK} **Secure Market:** Admin approval for Club Buys.\n{E_GOLD_TICK} **Human Numbers:** Support for 1.5k, 2m.")
