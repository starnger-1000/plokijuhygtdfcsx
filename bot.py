# bot.py
# Full Club Auction Bot (Final Production Build with Strict Wallet Logic)
...
E_BOX = "<a:itembox:1443254784898367629>" 
...
async def finalize_auction(item_type: str, item_id: int, channel_id: int):
    ...
    if winner_bid:
        ...
        if item_type == "club":
            ...
            # UPDATE: Market Value = Last Bid Amount & Transfer Ownership
            ...
            if channel:
                 await channel.send(embed=create_embed(
                    ...
                    f"{E_BOX} **Player:** {d_item['username']}\n"
...
async def groupinfo(ctx, *, group_name: str):
    ...
    for ex in ex_members:
         try: u = await bot.fetch_user(int(ex['user_id'])); name = u.name
         except: name = "Unknown"
         ex_list.append(name)
    if ex_list: embed.add_field(name="Ex-Members", value=", ".join(ex_list), inline=False)
...
async def placebid(ctx, amount: int, item_type: str, item_id: int, club_name: str = None):
    ...
    if item_type == "club":
         ...
         if c.get("owner_id") and not is_active:
             return await ctx.send(embed=create_embed(f"{E_ALERT} Sold Out", f"{E_ERROR} This club is **SOLD OUT**. Wait for owner to sell.", 0xff0000))
...
async def sellclub(ctx, club_name: str, buyer: discord.Member = None):
    ...
    if buyer:
        ...
        clubs_col.update_one({"id": c["id"]}, {
            "$set": {"owner_id": str(buyer.id), "ex_owner_id": old_owner}
        })
        profiles_col.update_one({"user_id": str(buyer.id)}, {"$set": {"owned_club_id": c["id"], "owned_club_share": 100}}, upsert=True)
    else:
        clubs_col.update_one({"id": c["id"]}, {
            "$set": {"owner_id": None, "ex_owner_id": old_owner}
        })
...
async def listduelists(ctx):
    ...
    for d in ds:
        ...
        if d.get("club_id"):
            ...
            if c: cname = c["name"]
        # FIX: E_ITEMBOX -> E_BOX
        embed.add_field(name=f"{d['username']}", value=f"{E_BOX} ID: {d['id']}\n{E_MONEY} ${d['expected_salary']:,}\n{E_STAR} {cname}", inline=True)
    await ctx.send(embed=embed)
...
async def clubinfo(ctx, *, club_name_or_id: str):
    ...
    embed.add_field(name="Ex-Owner", value=f"{E_RED_ARROW} {c.get('ex_owner_id', 'None')}", inline=True)
