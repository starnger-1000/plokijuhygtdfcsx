# bot.py
# Full Club Auction & Pokémon Shop Bot (Certified Final Production Build v3.3)
...
async def wallet(ctx):
    ...
    embed = create_embed(
        f"{E_MONEY} Wallet Balance", 
        f"**User:** {ctx.author.mention}\n\n{E_MONEY} **Cash:** ${cash:,}\n{E_PC} **PC:** {pc:,}\n{E_SHINY} **Shiny Coins:** {shiny:,}",
...
@bot.hybrid_command(name="shop_import", aliases=["si"], description="Admin: Add item to shop.")
@commands.has_permissions(administrator=True)
async def shop_import(ctx, name: str, price: int, type: str = "admin", is_shiny: bool = False, image: discord.Attachment = None):
    currency = "shiny" if type == "admin" else "pc"
    item_id = get_next_id("shop_item_id")
    image_url = image.url if image else None
    shop_items_col.insert_one({
        "id": item_id, "name": name, "price": price, "currency": currency,
        "seller_id": str(ctx.author.id) if type == "user" else "ADMIN",
        "is_shiny": is_shiny, "image_url": image_url, "sold": False, "timestamp": datetime.now()
    })
    curr_emoji = E_SHINY if currency == "shiny" else E_PC
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Item Added", f"**{name}** added to {type.title()} Shop.\n**Price:** {price:,} {curr_emoji}\n**ID:** {item_id}", 0x2ecc71, image=image_url))

@bot.hybrid_command(name="shop_remove", aliases=["sr"], description="Admin: Remove item from shop.")
@commands.has_permissions(administrator=True)
async def shop_remove(ctx, item_id: int):
    res = shop_items_col.delete_one({"id": item_id})
    if res.deleted_count > 0:
        await ctx.send(embed=create_embed(f"{E_SUCCESS} Removed", f"Item ID {item_id} removed from shop.", 0x2ecc71))
    else:
        await ctx.send(embed=create_embed("Error", "Item not found.", 0xff0000))

@bot.hybrid_command(name="shop_setprice", aliases=["ssp"], description="Admin: Change item price.")
@commands.has_permissions(administrator=True)
async def shop_setprice(ctx, item_id: int, new_price: int):
    res = shop_items_col.update_one({"id": item_id}, {"$set": {"price": new_price}})
    if res.modified_count > 0:
        await ctx.send(embed=create_embed(f"{E_SUCCESS} Updated", f"Item ID {item_id} price set to {new_price:,}.", 0x2ecc71))
    else:
        await ctx.send(embed=create_embed("Error", "Item not found.", 0xff0000))
...
# shiny_shop_part10.py

# ===========================
#   GROUP 3.6: SHOP USER & EXTRAS
# ===========================

@bot.hybrid_command(name="shop_list", aliases=["sl"], description="View Shop Items.")
async def shop_list(ctx, type: str = "admin"):
    currency = "shiny" if type.lower() == "admin" else "pc"
    items = list(shop_items_col.find({"currency": currency, "sold": False}))
    if not items: return await ctx.send(embed=create_embed(f"{E_ITEMBOX} Shop Empty", f"No items in {type} shop.", 0x95a5a6))
    data = []
    curr_emoji = E_SHINY if currency == "shiny" else E_PC
    for i in items:
        shiny_tag = "✨ " if i.get('is_shiny') else ""
        data.append((f"{shiny_tag}{i['name']}", f"**Price:** {i['price']:,} {curr_emoji}\n**ID:** {i['id']}"))
    view = Paginator(ctx, data, f"{E_ITEMBOX} {type.title()} Shop", 0x3498db)
    await ctx.send(embed=view.get_embed(), view=view)

@bot.hybrid_command(name="shop_buy", aliases=["sb"], description="Buy an item from shop.")
async def shop_buy(ctx, item_id: int):
    ...
    if item['seller_id'] != "ADMIN":
        after_tax = int(item['price'] * (1 - DEFAULT_TAX_RATE))
        wallets_col.update_one({"user_id": item['seller_id']}, {"$inc": {"pc": after_tax}})
        try:
            seller = await bot.fetch_user(int(item['seller_id']))
            await seller.send(embed=create_embed(f"{E_SUCCESS} Item Sold", f"Your **{item['name']}** sold!\nEarned: {after_tax:,} {E_PC} (Tax: {int(item['price']*DEFAULT_TAX_RATE):,})", 0x2ecc71))
        except: pass
    shop_items_col.update_one({"id": item_id}, {"$set": {"sold": True, "buyer_id": str(ctx.author.id), "sold_at": datetime.now()}})
    inventory_col.insert_one({"user_id": str(ctx.author.id), "item_name": item['name'], "price": item['price'], "currency": item['currency'], "timestamp": datetime.now()})
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Purchase Successful", f"You bought **{item['name']}**!", 0x2ecc71, image=item.get('image_url')))

@bot.hybrid_command(name="user_shop_add", aliases=["usa"], description="List item by showing PokéTwo Info.")
async def user_shop_add(ctx, price: int):
    await ctx.send(embed=create_embed(f"{E_PC} List Item", f"**Step 1:** Price set to **{price:,} {E_PC}**\n**Step 2:** Please run `p!info` (or `@PokéTwo info`) for the Pokémon you want to sell in this channel within 30 seconds.", 0x3498db))
    def check(m): return m.channel == ctx.channel and m.author.id == 716390085896962058 and m.embeds
    try:
        msg = await bot.wait_for('message', check=check, timeout=30)
        embed = msg.embeds[0]
        name = embed.title if embed.title else "Unknown Pokémon"
        image_url = embed.thumbnail.url if embed.thumbnail else (embed.image.url if embed.image else None)
        item_id = get_next_id("shop_item_id")
        shop_items_col.insert_one({
            "id": item_id, "name": name, "price": price, "currency": "pc",
            "seller_id": str(ctx.author.id), "is_shiny": False, "image_url": image_url,
            "sold": False, "timestamp": datetime.now()
        })
        await ctx.send(embed=create_embed(f"{E_SUCCESS} Item Listed", f"**{name}** listed!\n**Price:** {price:,} {E_PC}\n**ID:** {item_id}", 0x2ecc71, image=image_url))
    except asyncio.TimeoutError: await ctx.send(embed=create_embed("Timeout", f"{E_ERROR} No info message received.", 0xff0000))

@bot.hybrid_command(name="user_shop_remove", aliases=["usr"], description="Remove your listing.")
async def user_shop_remove(ctx, item_id: int):
    res = shop_items_col.delete_one({"id": item_id, "seller_id": str(ctx.author.id), "sold": False})
    if res.deleted_count > 0: await ctx.send(embed=create_embed(f"{E_SUCCESS} Removed", f"Item {item_id} removed from shop.", 0x2ecc71))
    else: await ctx.send(embed=create_embed("Error", "Item not found or you don't own it.", 0xff0000))

@bot.hybrid_command(name="inventory", aliases=["inv", "bag"], description="View bought items.")
async def inventory(ctx):
    items = list(inventory_col.find({"user_id": str(ctx.author.id)}).sort("timestamp", -1))
    if not items: return await ctx.send(embed=create_embed("Empty", "No items yet.", 0x95a5a6))
    data = []
    for i in items:
        emoji = E_SHINY if i.get('currency') == 'shiny' else E_PC
        data.append((i['item_name'], f"Bought for {i.get('price', 0):,} {emoji}"))
    view = Paginator(ctx, data, f"{E_ITEMBOX} Inventory", 0x3498db)
    await ctx.send(embed=view.get_embed(), view=view)

@bot.hybrid_command(name="daily", aliases=["claim"], description="Claim daily PC/Shiny.")
async def daily(ctx):
    ...
    pc_reward = 1000
    shiny_reward = 5
    wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {"pc": pc_reward, "shiny_coins": shiny_reward}, "$set": {"last_daily": datetime.now()}}, upsert=True)
    await ctx.send(embed=create_embed(f"{E_GIVEAWAY} Daily Claimed", f"+{pc_reward} {E_PC}\n+{shiny_reward} {E_SHINY}", 0x2ecc71))
...
async def create_coupon(ctx, code: str, discount_percent: int, uses: int):
    ...
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Coupon Created", f"Code: `{code}`\nDiscount: {discount_percent}%\nUses: {uses}", 0x2ecc71))

@bot.hybrid_command(name="create_redeem", aliases=["crc"], description="Admin: Create redeem code.")
@commands.has_permissions(administrator=True)
async def create_redeem(ctx, reward_type: str, amount: int):
    code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
    redeem_codes_col.insert_one({"code": code, "type": reward_type, "amount": amount, "claimed": False})
    await ctx.send(embed=create_embed(f"{E_GIVEAWAY} Code Generated", f"Code: `{code}`\nReward: {amount} {reward_type}", 0x2ecc71))

@bot.hybrid_command(name="redeem", aliases=["rcode"], description="Redeem a code.")
async def redeem(ctx, code: str):
    r = redeem_codes_col.find_one({"code": code, "claimed": False})
    if not r: return await ctx.send(embed=create_embed("Error", "Invalid code.", 0xff0000))
    redeem_codes_col.update_one({"_id": r['_id']}, {"$set": {"claimed": True, "claimer": str(ctx.author.id)}})
    field = "pc" if r['type'] == "pc" else "shiny_coins"
    wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {field: r['amount']}}, upsert=True)
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Redeemed", f"You got **{r['amount']} {r['type'].upper()}**!", 0x2ecc71))

@bot.hybrid_command(name="mystery_buy", aliases=["mb"], description="Buy Mystery Box.")
async def mystery_buy(ctx, type: str):
    t = type.lower()
    if t in ['s', 'shiny']: key = 'shiny'
    elif t in ['r', 'rare']: key = 'rare'
    elif t in ['c', 'common']: key = 'common'
    elif t in ['reg', 'regional']: key = 'regional'
    else: return await ctx.send(embed=create_embed("Error", "Types: shiny, rare, common, regional", 0xff0000))
    box = MYSTERY_CONFIG[key]
    price = box['price']
    w = wallets_col.find_one({"user_id": str(ctx.author.id)})
    if not w or w.get("shiny_coins", 0) < price: return await ctx.send(embed=create_embed("Insufficient Funds", f"Need **{price:,} {E_SHINY}**.", 0xff0000))
    wallets_col.update_one({"user_id": str(ctx.author.id)}, {"$inc": {"shiny_coins": -price}})
    await ctx.send(embed=create_embed(f"{box['emoji']} Mystery Box", f"You bought a **{box['name']}**!\nCheck inventory.", 0x9b59b6))
...
class DepositView(discord.ui.View):
    def __init__(self, ctx, amount):
        ...
        self.amount = amount
        self.value = None
    @discord.ui.button(label="Confirm", style=discord.ButtonStyle.green, emoji=discord.PartialEmoji.from_str(E_GOLD_TICK))
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.ctx.author.id: return
        self.value = True
        self.stop()
        await interaction.response.defer()
    @discord.ui.button(label="Cancel", style=discord.ButtonStyle.red, emoji=discord.PartialEmoji.from_str(E_ERROR))
...
async def pc_deposit(ctx):
    await ctx.send(embed=create_embed(f"{E_PC} PC Deposit", "Please run `p!bal` now.", 0x3498db))
    def check(m): return m.channel == ctx.channel and m.author.id == 716390085896962058 and (str(ctx.author.id) in m.content or ctx.author.name in m.content or m.reference)
    try:
        msg = await bot.wait_for('message', check=check, timeout=30)
        content = msg.content.lower().replace(",", "")
        match = re.search(r'(\d+)\s*pc', content)
        ...
        if view.value:
            ...
            log_user_activity(ctx.author.id, "Deposit", f"Deposited {amount:,} PC")
            await conf_msg.edit(embed=create_embed(f"{E_SUCCESS} Successful", f"Added **{amount:,} {E_PC}**.", 0x2ecc71), view=None)
        else: await conf_msg.edit(embed=create_embed("Cancelled", "Deposit cancelled.", 0x95a5a6), view=None)
    except asyncio.TimeoutError: await ctx.send(embed=create_embed("Timeout", "Session expired.", 0xff0000))
...
class HelpView(discord.ui.View):
    ...
    async def market_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await self.send_category_embed(interaction, f"{E_AUCTION} Club Market", "Buy, Sell & Trade.", fields, 0xe67e22)
    ...
    async def shop_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        fields = [
            (f"{E_SHINY} **Admin Shop**", f"`.sl admin` **List:** View Admin items.\n`.sb <ID>` **Buy:** Purchase with Shiny Coins."),
            (f"{E_PC} **User Shop**", f"`.sl user` **List:** View Player items.\n`.usa <Price>` **Add:** List item (Requires p!info).\n`.usr <ID>` **Remove:** Delist item."),
            (f"{E_ITEMBOX} **Inventory & Rewards**", f"`.inv` **Inventory:** View bought items.\n`.pcd` **Deposit:** Add PC from PokéTwo.\n`.daily` **Daily:** Claim reward.\n`.mb <Type>` **Mystery Box:** Buy box.\n`.rcode <Code>` **Redeem:** Claim code.")
        ]
        await self.send_category_embed(interaction, f"{E_PC} Pokémon Market", "Buy & Sell Items.", fields, 0x9b59b6)
    ...
    async def admin_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        fields = [
            (f"{E_ADMIN} **Management**", f"`.cd` **Check Deals** | `.md` **Manage Deal**\n`.rc` **Register Club** | `.dc` **Delete Club**\n`.sr <ID>` **Remove Shop Item**\n`.ssp <ID> <Price>` **Set Shop Price**"),
            (f"{E_AUCTION} **Auctions**", f"`.sca` **Club Auction** | `.sda` **Duelist Auction**\n`.fa` **Freeze** | `.ufa` **Unfreeze**"),
            (f"{E_MONEY} **Economy**", f"`.tp` **Tip** | `.du` **Deduct** | `.agf` **Group Fund** | `.po` **Payout**\n`.cc` **Create Coupon** | `.crc` **Create Redeem**"),
    ...
    @discord.ui.button(label="Updates (v3.2)", style=discord.ButtonStyle.primary, emoji=discord.PartialEmoji.from_str(E_BOOST))
    async def updates_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        desc = (
            f"{E_STARS} **Latest Updates**\n"
            f"{E_GOLD_TICK} **Full Shop System:** User Listings (via p!info), Admin Management.\n"
            f"{E_GOLD_TICK} **New Commands:** `.usa` (List Item), `.usr` (Remove), `.daily`.\n"
            f"{E_GOLD_TICK} **Human Numbers:** Support for `k`, `m`, `b`.\n"
            f"{E_GOLD_TICK} **Secure Market:** Approvals for Club Buys.\n"
            f"{E_GOLD_TICK} **Adv. Giveaways:** Images & Auto-tipping."
        )
...
async def botinfo(ctx):
    embed = discord.Embed(
        title=f"{E_CROWN} **Ze Bot: Info & Shop**",
        description=(
            f"**Welcome to the ultimate Football & Pokémon Economy!**\n\n"
            f"**{E_SHINY} Shiny / Rare Pokémon Shop**\n"
            f"• **Admin Shop:** Buy rare items using Shiny Coins {E_SHINY}.\n"
            f"• **User Shop:** Buy/Sell Pokémon using PC {E_PC}.\n"
            f"• **Mystery Boxes:** Try your luck!\n\n"
            f"**{E_FIRE} Football Market**\n"
            f"• Manage clubs, bid on auctions, and win matches.\n\n"
            f"**Currency Exchange:**\n"
            f"100 {E_MONEY} = 1 {E_SHINY} (Rate may vary)\n\n"
            f"{E_ADMIN} **Credits:** Created by **Soul Gill**."
        ),
        color=0xf1c40f
    )
    if bot.user.avatar: embed.set_thumbnail(url=bot.user.avatar.url)
    embed.add_field(name=f"{E_BOOK} **Navigation**", value="Click the buttons below to view the full command list.", inline=False)
    view = HelpView()
