...
# Part 1 of 2 - Core Systems & Markets
...
def get_level_info(current_wins, level_name=None):
    ...
    for wins_required, name, bonus in LEVEL_UP_CONFIG:
        if wins_required > current_wins:
            ...
            current_level = name
    return current_level, next_level_info, 0 # Simplified return
    ...
    if owner_display.startswith('group:'): 
        ...
        try:
            if owner_display != "Unowned":
                u = await bot.fetch_user(int(owner_display))
                owner_display = f"User: {u.display_name}"
        except: pass
...
async def deductsalary(ctx, duelist_id: int, confirm: str):
    ...
    if str(ctx.author.id) not in owner_ids: return await ctx.send(embed=create_embed("Error", "Not authorized.", 0xff0000))
...
async def managedeal(ctx, deal_id: int, action: str):
    ...
    if action == "reject":
        if deal['type'] == "group": groups_col.update_one({"name": deal['buyer_id'].replace("group:", "")}, {"$inc": {"funds": deal['price']}})
        else: wallets_col.update_one({"user_id": deal['buyer_id']}, {"$inc": {"balance": deal['price']}})
        pending_deals_col.delete_one({"id": deal_id})
        return await ctx.send(embed=create_embed(f"{E_SUCCESS} Rejected", f"Deal #{deal_id} rejected. Funds refunded.", 0x2ecc71))
    if action == "approve":
        clubs_col.update_one({"id": c["id"]}, {"$set": {"owner_id": deal['buyer_id']}})
        if deal['type'] == "user": profiles_col.update_one({"user_id": deal['buyer_id']}, {"$set": {"owned_club_id": c["id"], "owned_club_share": 100}}, upsert=True)
        history_col.insert_one({"club_id": c["id"], "winner": deal['buyer_id'], "amount": deal['price'], "timestamp": datetime.now(), "type": "market_buy"})
        pending_deals_col.delete_one({"id": deal_id})
...
@commands.has_permissions(administrator=True)
async def registerclub(ctx, name: str, base_price: HumanInt):
    ...
    clubs_col.insert_one({"id": cid, "name": name, "base_price": base_price, "value": base_price, "logo": logo_url, "total_wins": 0, "level_name": LEVEL_UP_CONFIG[0][1], "owner_id": None, "manager_id": None})
    ...
    if reaction:
        async for user in reaction.users():
            if not user.bot:
                ...
                if member:
                    users.append(member)
    if not users: return await msg.reply(embed=create_embed("Ended", "No entrants.", 0x95a5a6))
...
class ShopCategoryView(discord.ui.View):
    ...
    async def show_category(self, interaction, category):
        ...
        # Filter for category
        # Note: User items might not have category unless specified, we assume manual entry
        # For Admin: We trust input. For User: We filter.
        
        # Simplified filter logic: fetch all of currency and manual filter or fetch specific
        # To keep it simple for now, we fetch all of currency and filter in python if needed, 
        # or assume 'category' field exists in DB. Let's add category to .si and .usa
        
        items = list(shop_items_col.find({"currency": currency, "sold": False, "category": category}))
        
        if not items:
            return await interaction.response.send_message(f"No items in **{category.title()}** category.", ephemeral=True)
            
        data = []
        for i in items:
            # For users, we extracted name from embed, so it is trustworthy
            data.append((f"{i['name']}", f"**Price:** {i['price']:,} {emoji_c}\n**ID:** {i['id']}"))
    ...
    async def shiny(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.show_category(interaction, "shiny")
    ...
    async def rare(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.show_category(interaction, "rare")
    ...
    async def regional(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.show_category(interaction, "regional")
    ...
    async def common(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.show_category(interaction, "common")
...
class ShopTypeView(discord.ui.View):
    ...
    async def admin_shop(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(embed=create_embed(f"{E_SHINY} Admin Shop", "Select a category:", 0xe74c3c), view=view, ephemeral=True)
    ...
    async def user_shop(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message(embed=create_embed(f"{E_PC} User Shop", "Select a category:", 0x2ecc71), view=view, ephemeral=True)

@bot.hybrid_command(name="shop", description="Open the Pokémon Shop Menu.")
async def shop(ctx):
    view = ShopTypeView(ctx)
    await ctx.send(embed=create_embed(f"{E_ITEMBOX} Pokémon Market", "Select a shop type to browse:", 0x3498db), view=view)
...
async def shop_import(ctx, name: str, price: int, category: str, is_shiny: bool = False, image: discord.Attachment = None):
    ...
    shop_items_col.insert_one({
        "id": item_id, "name": name, "price": price, "currency": "shiny",
        "seller_id": "ADMIN", "is_shiny": is_shiny, "image_url": image_url, "sold": False, "timestamp": datetime.now(),
        "category": cat
    })
...
@bot.hybrid_command(name="botinfo", aliases=["info"], description="Info Panel.")
async def botinfo(ctx):
    embed = discord.Embed(title=f"{E_CROWN} Ze Bot", description=f"**Football & Pokémon Economy**\n\n{E_SHINY} **Shiny Shop**\n{E_PC} **User Shop**\n{E_FIRE} **Football**", color=0xf1c40f)
    view = HelpView()
