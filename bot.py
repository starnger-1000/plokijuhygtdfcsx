# bot.py
# Full Club Auction Bot (Final Master Build - Verified & Deduplicated)
...
class BotInfoView(View):
    ...
    async def market(self, interaction, button):
        ...
        **{E_AUCTION} Market Commands**
        ...
        `/clubinfo` or `.ci` - View club stats.
        `/clublevel` - View club level/division.
        `/marketpanel` - Graphical market overview.
        `/leaderboard` or `.lb` - View top clubs.
        """
    ...
    async def duelists(self, interaction, button):
        ...
        **{E_ITEMBOX} Duelist Commands**
        ...
        `/adjustsalary` or `.adj` - Owner only: Bonus/Fine.
        `/deductsalary` - Owner only: Match penalty.
        """
...
async def battleresult(ctx, battle_id: int, winner_name: str):
    ...
    embed_log = create_embed(f"{E_FIRE} Match Result", f"{win_trophy} **Winner:** {wc['name']} {winner_emoji}\n{lose_mark} **Loser:** {lc['name']} {loser_emoji}\n\n_{final_banter}_", 0xe74c3c)
