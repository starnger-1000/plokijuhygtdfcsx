# bot.py
# Full Club Auction Bot (Certified Final Build)
...
# Log Channels
LOG_CHANNELS = {
    "withdraw": 1443955967086690395, # Payment/Payout Logs
    ...
    if winner_bid:
        ...
        if item_type == "club":
            ...
            if old_owner and not old_owner.startswith("group:"):
                ...
                log_user_activity(old_owner, "Sale", f"Sold club {club_item['name']} via Auction.")
            ...
            if bidder_str.startswith('group:'):
                ...
                if c: target_club_id = c['id']
                log_user_activity(bidder_str, "Win", f"Won Auction for Duelist {d_item['username']}")
...
async def groupinfo(ctx, *, group_name: str):
    ...
    # SAFE TRUNCATION to prevent Embed Errors
...
async def battleresult(ctx, battle_id: int, winner_name: str):
    ...
    embed_log = create_embed(f"{E_FIRE} Match Result", f"🏆 **Winner:** {wc['name']}\n💩 **Loser:** {lc['name']}\n\n_{banter}_", 0xe74c3c)
