# bot.py
# Full Club Auction Bot (MongoDB + Premium Embeds)
...
# ---------- EMOJI CONFIG ----------
# Using strings directly to make embedding easier
E_ACTIVE = "<a:geeen_dot:1417645972787298355>"
E_DANGER = "<a:red_dot:1417646052915548263>"
E_ALERT = "<a:alert:1417586766331777134>" # Red alert alarm
...
E_BLACK_BOOST = "<a:boost:1416878722270630031>"
E_PIKACHU = "<a:miapikachu:1430351344451063839>"
E_MONEY = "<a:Donation:962944611792326697>"
E_GIVEAWAY = "<a:Giveaway2:954896141382725652>"
...
E_ARROW = "<a:arrow_arrow:962945777821450270>" # Golden arrow
E_RED_ARROW = "<a:redarrow:1424106887103905892>"
E_STAR = "<a:yellowstar:1431126619795488791>"
E_AUCTION = "<:Auction:1392502904677601503>"
E_ADMIN_STATIC = "<:Administrator:1383057812581781627>"
E_CHAT = "<a:text:1396756733690646649>"
...
def update_club_level(club_id, wins_gained=0):
    ...
    for wins_required, name, bonus in LEVEL_UP_CONFIG:
        if c.get("total_wins", 0) < wins_required <= new_wins:
            clubs_col.update_one({"id": int(club_id)}, {
                "$set": {"level_name": name},
                "$inc": {"value": bonus}
            })
    ...
    if winner_bid:
        ...
        if item_type == "club":
            history_col.insert_one({
                ...
                "market_value_at_sale": club_item.get("value", 0)
            })
            clubs_col.update_one({"id": int(item_id)}, {
                "$set": {"owner_id": bidder_str, "last_bid_price": amount}
            })
            if not bidder_str.startswith('group:'):
                profiles_col.update_one(
                    {"user_id": bidder_str},
                    {"$set": {"owned_club_id": int(item_id), "owned_club_share": 100}},
                    upsert=True
                )
                
            if channel:
                embed = create_embed(
                    f"{E_AUCTION} Auction Concluded", 
                    f"{E_SUCCESS} Winner: **{bidder_str}**\n{E_MONEY} Amount: **${amount:,}**\n{E_ITEMBOX} Item: **{club_item['name']}**",
                    color=0xf1c40f
                )
            ...
            contracts_col.insert_one({
                "duelist_id": int(item_id),
                "club_owner": bidder_str,
            ...
            duelists_col.update_one({"id": int(item_id)}, {
                "$set": {"owned_by": bidder_str, "club_id": target_club_id}
            })
            
            if channel:
                 embed = create_embed(
                    f"{E_AUCTION} Duelist Signed", 
        ...
        if channel: 
            embed = create_embed(f"{E_TIMER} Auction Ended", "No bids were placed.", color=0x95a5a6)
            await channel.send(embed=embed)
...
async def registerclub(ctx, name: str, base_price: int, *, slogan: str = ""):
    if clubs_col.find_one({"name": {"$regex": f"^{name}$", "$options": "i"}}):
        embed = create_embed("Error", f"{E_ERROR} Club already registered.", 0xff0000)
        return await ctx.send(embed=embed)
        
    logo_url = ctx.message.attachments[0].url if ctx.message.attachments else ""
    cid = get_next_id("club_id")
    
    clubs_col.insert_one({
        "id": cid, "name": name, "base_price": base_price, "value": base_price,
        "slogan": slogan, "logo": logo_url, "total_wins": 0,
        "level_name": LEVEL_UP_CONFIG[0][1], "owner_id": None, "manager_id": None
    })
    
    embed = create_embed(
        f"{E_SUCCESS} Club Registered",
        f"{E_ARROW} **Name:** {name}\n{E_MONEY} **Base Price:** ${base_price:,}\n{E_ITEMBOX} **ID:** {cid}",
        0x2ecc71, thumbnail=logo_url
    )
    await ctx.send(embed=embed)
    log_audit(f"{ctx.author} registered club {name}")

@bot.command()
async def listclubs(ctx):
    clubs = list(clubs_col.find().sort("value", -1).limit(20))
    if not clubs:
        return await ctx.send(embed=create_embed("Info", "No clubs found.", 0x95a5a6))
        
    embed = discord.Embed(title=f"{E_CROWN} Registered Clubs (Top 20)", color=0x3498db)
    for c in clubs:
        embed.add_field(
            name=f"{E_STAR} {c['name']} (ID: {c['id']})",
            value=f"{E_MONEY} Val: ${c['value']:,}\n{E_BOOST} {c.get('level_name', 'Unranked')}\n{E_FIRE} Wins: {c.get('total_wins',0)}",
            inline=False
        )
    await ctx.send(embed=embed)
...
async def startclubauction(ctx, club_name: str):
    ...
    embed = create_embed(
        f"{E_AUCTION} Auction Started!",
        f"{E_ARROW} **Club:** {c['name']}\n{E_MONEY} **Base Price:** ${c['base_price']:,}\n{E_TIMER} Use `.placebid` to participate!",
        0xe67e22, thumbnail=c.get('logo')
    )
    await ctx.send(embed=embed)
...
async def registerduelist(ctx, username: str, base_price: int, salary: int):
    if duelists_col.find_one({"discord_user_id": str(ctx.author.id)}):
        return await ctx.send(embed=create_embed("Error", f"{E_ERROR} Already registered.", 0xff0000))
        
    did = get_next_id("duelist_id")
    avatar = ctx.author.avatar.url if ctx.author.avatar else ""
    
    duelists_col.insert_one({
        "id": did, "discord_user_id": str(ctx.author.id), "username": username,
        "base_price": base_price, "expected_salary": salary, "avatar_url": avatar,
        "owned_by": None, "club_id": None
    })
    embed = create_embed(
        f"{E_SUCCESS} Duelist Registered",
        f"{E_ARROW} **Name:** {username}\n{E_ITEMBOX} **ID:** {did}\n{E_MONEY} **Price:** ${base_price:,}",
        0x9b59b6, thumbnail=avatar
    )
    await ctx.send(embed=embed)
...
async def startduelistauction(ctx, duelist_id: int):
    ...
    embed = create_embed(
        f"{E_AUCTION} Duelist Auction!",
        f"{E_ARROW} **Player:** {d['username']}\n{E_MONEY} **Base Price:** ${d['base_price']:,}\n{E_TIMER} **ID:** {d['id']}",
        0x9b59b6, thumbnail=d.get('avatar_url')
    )
    await ctx.send(embed=embed)
...
async def placebid(ctx, amount: int, item_type: str, item_id: int, club_name: str = None):
    if bidding_frozen: return await ctx.send(embed=create_embed("Frozen", f"{E_DANGER} Auctions are frozen.", 0xff0000))
    
    item_type = item_type.lower()
    if item_type == "duelist":
        if not club_name: return await ctx.send(embed=create_embed("Error", f"{E_ALERT} Provide club name for duelist bid.", 0xff0000))
        c = clubs_col.find_one({"name": {"$regex": f"^{club_name}$", "$options": "i"}})
        if not c: return await ctx.send(embed=create_embed("Error", f"{E_ERROR} Club not found.", 0xff0000))
        if str(ctx.author.id) != c.get("owner_id"):
            return await ctx.send(embed=create_embed("Error", f"{E_ERROR} You don't own this club.", 0xff0000))
            
    uid = str(ctx.author.id)
    w = wallets_col.find_one({"user_id": uid})
    bal = w["balance"] if w else 0
    if bal < amount: return await ctx.send(embed=create_embed("Funds", f"{E_ERROR} Insufficient funds.", 0xff0000))
    
    curr = get_current_bid(item_type, item_id)
    req = min_required_bid(curr)
    if amount < req:
        return await ctx.send(embed=create_embed("Bid Error", f"{E_RED_ARROW} Min bid is **${req:,}**.", 0xff0000))
        
    bids_col.insert_one({"bidder": uid, "amount": amount, "item_type": item_type, "item_id": int(item_id), "timestamp": datetime.now()})
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Bid Placed", f"{E_ARROW} Bid of **${amount:,}** accepted.", 0x2ecc71))
...
async def groupbid(ctx, group_name: str, amount: int, item_type: str, item_id: int, club_name: str = None):
    ...
    # Duelist logic same as placebid
    if item_type == "duelist":
        if not club_name: return await ctx.send(embed=create_embed("Error", f"{E_ALERT} Provide club name.", 0xff0000))
        c = clubs_col.find_one({"name": {"$regex": f"^{club_name}$", "$options": "i"}})
        if not c or c.get('owner_id') != f"group:{group_name.lower()}":
            return await ctx.send(embed=create_embed("Error", f"{E_ERROR} Group doesn't own this club.", 0xff0000))
    ...
    if not mem: return await ctx.send(embed=create_embed("Error", f"{E_ERROR} Not a member.", 0xff0000))
    
    if g["funds"] < amount: return await ctx.send(embed=create_embed("Funds", f"{E_ERROR} Insufficient group funds.", 0xff0000))
    
    curr = get_current_bid(item_type, item_id)
    req = min_required_bid(curr)
    if amount < req: return await ctx.send(embed=create_embed("Bid Error", f"{E_RED_ARROW} Min bid is **${req:,}**.", 0xff0000))
    
    bids_col.insert_one({"bidder": f"group:{gname}", "amount": amount, "item_type": item_type, "item_id": int(item_id), "timestamp": datetime.now()})
    await ctx.send(embed=create_embed(f"{E_SUCCESS} Group Bid", f"{E_ARROW} Group **{group_name}** bid **${amount:,}**.", 0x2ecc71))
    schedule_auction_timer(item_type, item_id, ctx.channel.id)

@bot.command()
async def clubinfo(ctx, *, club_name_or_id: str):
    try:
        c = clubs_col.find_one({"id": int(club_name_or_id)})
    except:
        c = clubs_col.find_one({"name": {"$regex": f"^{club_name_or_id}$", "$options": "i"}})
        
    if not c: return await ctx.send(embed=create_embed("Error", f"{E_ERROR} Club not found.", 0xff0000))
    
    owner_display = c['owner_id'] or "Unowned"
    if owner_display.startswith('group:'): owner_display = f"Group: {owner_display.replace('group:', '').title()}"
    
    manager_name = "None"
    if c['manager_id']:
        try: 
            u = await bot.fetch_user(int(c['manager_id']))
            manager_name = u.name
        except: pass
...
async def marketpanel(ctx, *, club_name_or_id: str):
    ...
    embed.add_field(name="Current Division", value=f"{E_CROWN} **{cur_lvl}**", inline=False)
    
    if nxt_lvl:
        embed.add_field(name="Next Division", value=f"{E_ARROW} **{nxt_lvl[0]}**", inline=True)
        embed.add_field(name="Wins Required", value=f"{E_RED_ARROW} **{req_wins}** more", inline=True)
        embed.add_field(name="Bonus", value=f"{E_PREMIUM} **+${nxt_lvl[2]:,}**", inline=True)
    else:
         embed.add_field(name="Status", value=f"{E_GOLD_TICK} **Max Level Reached**", inline=False)
...
async def sellclub(ctx, club_name: str, buyer: discord.Member = None):
    ...
    confirm_msg = await ctx.send(embed=create_embed(f"{E_ALERT} Confirm Sale", f"Sell **{c['name']}** to {target.mention if buyer else 'Market'} for **${val:,}**?\nType `yes` or `no`.", 0xe67e22))
    ...
    if msg.content.lower() == 'no': return await ctx.send("Cancelled.")
    
    # Process
    if buyer:
        # Check buyer funds
        bw = wallets_col.find_one({"user_id": str(buyer.id)})
        if not bw or bw["balance"] < val: return await ctx.send("Buyer insufficient funds.")
...
async def sellshares(ctx, club_name: str, buyer: discord.Member, percentage: int):
    # Simplified logic to save space, full logic matches previous request
    # Checks shares -> asks buyer -> transfers money -> updates shares
    c = clubs_col.find_one({"name": {"$regex": f"^{club_name}$", "$options": "i"}})
    if not c: return await ctx.send("Club not found.")
    
    owner_str = c.get("owner_id", "")
    if not owner_str.startswith("group:"): return await ctx.send("Not group owned.")
    gname = owner_str.replace("group:", "")
    
    seller = group_members_col.find_one({"group_name": gname, "user_id": str(ctx.author.id)})
    if not seller or seller["share_percentage"] < percentage: return await ctx.send("Not enough shares.")
    
    val = int(c["value"] * (percentage / 100))
    
    await ctx.send(f"{buyer.mention}, buy **{percentage}%** shares for **${val:,}**? `yes`/`no`")
    ...
    if msg.content.lower() == 'yes':
        bw = wallets_col.find_one({"user_id": str(buyer.id)})
        if not bw or bw["balance"] < val: return await ctx.send("Broke.")
        ...
        group_members_col.update_one(
            {"group_name": gname, "user_id": str(buyer.id)},
            {"$inc": {"share_percentage": percentage}},
            upsert=True
        )
...
async def registerbattle(ctx, club_a_name: str, club_b_name: str):
    ...
    if not ca or not cb: return await ctx.send("Clubs not found.")
...
async def battleresult(ctx, battle_id: int, winner_name: str):
    ...
    if not b: return await ctx.send("Battle not found.")
    
    wc = clubs_col.find_one({"name": {"$regex": f"^{winner_name}$", "$options": "i"}})
    if not wc: return await ctx.send("Winner not found.")
